...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.forecasts.create') }}">
        ...
        <div class="row col-md-6">
            ...
            <div class="col-4">
                <input type="date" name="date" class="form-control mb-3" value="{{ \Illuminate\Support\Carbon::now()->format('Y-m-d') }}">
            </div>
    ...
    @foreach($cities as $city)
        <div class="mb-5 mx-2">
            ...
            @foreach($city->forecasts as $forecast)
                ...
                <li class="list-group-item">{{ $forecast->date }} - <span style="color:{{ $color }};">{{ $forecast->temperature }}</span> - {{ $forecast->weather_type }} - {{ $forecast->probability }}</li>
            @endforeach
            </ul>
        </div>
    @endforeach
